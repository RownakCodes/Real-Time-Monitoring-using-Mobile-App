...
void readTemperatures(){
   Serial.println("Read sensors' values...");
      ...
      for (int i = 0; i < 9; i++){
        ...
        for (int j = i + 1; j < 10; j++){ //binary sort
          if (buf[i] > buf[j]  )
          {
            temp = buf[i];
            buf[i] = buf[j];
      ...
      avgValue = 0;
      for(int i = 2; i < 8; i++){ // avg of mid 6
        avgValue += buf[i];
      }
      float phValue = (float)avgValue*5.0/1024/6; // Conver analog to mili volt
      phValue = 3.5*phValue;  //convert milivolt into PH
      ...
      if (isnan(temperature) || isnan(humidity || isnan(phValue))) {
        Serial.println("Failed to read from DHT");
          ...
          Serial.println("Humidity="+String(phValue)+" unit");
          ...
          virtuino.vMemoryWrite(1,humidity);    // write humidity to virtual pin V1. On Virtuino panel add a value display or an analog instrument to pin V1
          virtuino.vMemoryWrite(2,phValue);  // write phVal on V2
      }
